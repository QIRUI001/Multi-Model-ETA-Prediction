...
\keywords{vessel ETA prediction, AIS data, graph neural networks, knowledge distillation, conformal prediction}
...
% Identify the gap
To address these challenges, we study full-voyage trans-Pacific vessel ETA prediction as a large-scale geospatial forecasting problem over AIS trajectories. We introduce MSTGN, a graph-augmented ETA prediction model that exploits maritime route structure by discretizing ocean space into grid cells, constructing a route graph from AIS transitions and geographic adjacency, and fusing GCN-derived spatial context with navigation and meteorological features. Based on this framework, we conduct a controlled benchmark of ten representative forecasting models across five architectural families and evaluate both prediction accuracy and operational reliability. Our primary contributions are fourfold:

\begin{enumerate}
    \item We formulate full-voyage trans-Pacific vessel ETA prediction as a large-scale geospatial forecasting problem, using 35.9 million AIS position reports and 9,087 voyages to evaluate ETA models under a unified and reproducible experimental protocol.

    \item We propose MSTGN, a graph-augmented ETA prediction model that discretizes ocean space into route-aware grid cells, constructs a maritime route graph from AIS transitions and geographic adjacency, and fuses GCN-derived spatial context with enriched navigation and meteorological features.

    \item We provide a controlled empirical study of ten representative forecasting models across five architectural families, showing that graph-augmented statistical feature fusion and ensemble knowledge distillation achieve strong single-model accuracy while avoiding multi-model inference at deployment.

    \item We analyze operational reliability beyond average error, including large-deviation failures on ultra-long voyages and ensemble-based conformal prediction intervals, and derive duration-specific buffer times for port scheduling under uncertainty.
\end{enumerate}
