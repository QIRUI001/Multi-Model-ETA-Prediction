...
Accurate ETAs enable port operators to optimize berth allocation, coordinate crane assignments, reduce vessel waiting times, and improve the resilience of maritime supply chains~\cite{unctad2023}.
With the widespread deployment of the Automatic Identification System (AIS), large volumes of vessel position, speed, and course records have become available for data-driven ETA prediction~\cite{fang2022}.
For long-haul ocean shipping, however, ETA prediction is not only a time-series regression task.
It is a geospatial forecasting problem over sparse AIS trajectories, route-constrained vessel movement, open-ocean corridors, and duration-dependent operational uncertainty.

Full-voyage trans-Pacific ETA prediction is especially challenging because vessels may be thousands of nautical miles from the destination when predictions are made.
At this scale, prediction quality depends not only on recent motion patterns but also on the vessel's spatial position within maritime corridors, the remaining route geometry, heterogeneous weather and navigation conditions, and the varying uncertainty of short-, medium-, and long-duration voyages.
These characteristics make open-ocean ETA prediction different from near-port arrival prediction and require models that can represent both statistical voyage features and spatial route structure.

Recent years have seen a wide range of machine learning methods applied to vessel trajectory and ETA prediction.
Recurrent neural networks such as LSTM~\cite{zhang2022lstm} and GRU~\cite{han2021gru} model sequential vessel dynamics, convolutional approaches such as TCN~\cite{bai2018tcn} capture local temporal patterns, and Transformer-based architectures~\cite{dong2023transformer, zhou2021informer} use attention mechanisms to model long-range dependencies.
At the same time, gradient-boosted tree methods such as XGBoost~\cite{chen2016xgboost} remain highly competitive when strong tabular and statistical features are available.
Despite this diversity, existing studies are difficult to compare directly because they often differ in route scope, prediction horizon, preprocessing pipeline, input representation, and evaluation protocol.
Recent time-series forecasting surveys~\cite{kim2025comprehensive, zeng2023transformers} similarly caution that complex deep architectures do not always outperform simpler baselines under careful evaluation.

To address these challenges, we study full-voyage trans-Pacific vessel ETA prediction as a large-scale geospatial forecasting problem over AIS trajectories.
We introduce MSTGN, a graph-augmented ETA prediction framework that discretizes ocean space into route-aware grid cells, constructs a maritime route graph from observed AIS transitions and geographic adjacency, and fuses GCN-derived spatial context with enriched navigation and meteorological features.
Rather than claiming to isolate architecture effects completely, we compare representative ETA prediction pipelines under a shared data split, target definition, preprocessing protocol, and evaluation metrics.
This design allows us to examine which modeling ingredients are most useful for long-distance maritime ETA prediction while acknowledging that different model families rely on different input representations.

Our primary contributions are fourfold:
\begin{enumerate}
    \item We formulate full-voyage trans-Pacific vessel ETA prediction as a large-scale geospatial forecasting problem, using 35.9 million AIS position reports and 9,087 voyages to evaluate ETA prediction pipelines under a shared experimental protocol.

    \item We propose MSTGN, a route-aware graph-augmented prediction framework that represents ocean space through grid cells, models maritime connectivity through transition and geographic edges, and integrates spatial graph embeddings with statistical navigation and weather features.

    \item We provide a comparative study of ten representative ETA prediction pipelines across five model families. The results show that strong tabular baselines remain difficult to outperform, while MSTGN with knowledge distillation achieves comparable accuracy to strong tree-based baselines and supports deployable single-model inference.

    \item We analyze operational reliability beyond average prediction error, including large-deviation failures on ultra-long voyages and ensemble-based conformal prediction intervals, and derive duration-specific buffer times for port scheduling under uncertainty.
\end{enumerate}

\section{Related Work}

Existing vessel ETA prediction studies can be broadly grouped by how they use vessel movement information.
Path-based methods first construct, search, or match a likely route and then estimate the remaining travel time along that route.
Trajectory-tracking methods instead learn arrival time from AIS trajectories, vessel states, and operational covariates.
AIS data provides dynamic movement attributes such as location, speed, and heading, together with static vessel information~\cite{yangHarnessingPowerMachine2024}; recent reviews show that these data have supported a broad range of ETA prediction models across ports, waterways, and maritime logistics settings~\cite{jiangPredictionVesselArrival2025,nomanReviewVesselTime2025}.
We review these two dominant branches before summarizing the remaining gaps for full-voyage open-ocean ETA prediction.

\subsection{Path-based Vessel ETA Prediction}
Path-based methods preserve explicit spatial route information rather than treating ETA as a purely tabular regression problem.
Park et al.~\cite{parkVesselEstimatedTime2021} combined AIS route grids, Q-learning path search, and MCMC-based speed sampling to stabilize vessel ETA prediction around Busan Port.
Yoon et al.~\cite{yoonEnhancingContainerVessel2023} constructed representative historical routes from prior port calls and mapped real-time AIS positions to route segments for container-vessel ETA prediction at Busan New Port.
Wenzel et al.~\cite{wenzelNeuralNetworkApproach2023} also incorporated route-distance estimation into neural ETA prediction for inland waterway transport.
More recently, Lloret-Batlle et al.~\cite{lloretbatlle2025} studied cross-Pacific ETA prediction using AIS trajectory data, further illustrating the importance of long-distance spatial context.
As reviewed by Jiang et al.~\cite{jiangPredictionVesselArrival2025}, path-based approaches are most effective when route structure is sufficiently regular and historical route patterns can be reused.

\subsection{Trajectory-tracking Methods for Vessel ETA Prediction}
Trajectory-tracking methods use AIS positions, vessel states, and operational covariates to estimate ETA directly from observed movement patterns.
Early operational studies showed that machine-learned ETA can support terminal decisions: Yu et al.~\cite{yuShipArrivalPrediction2018} used RF, ANN, and CART models for ship arrival prediction, while Chu et al.~\cite{chuEvaluationPredictionPunctuality2023} evaluated vessel-reported AIS and port-call information for arrival punctuality prediction.
The branch has since evolved toward multi-source fusion and ensemble learning.
Kolley et al.~\cite{kolleyRobustBerthScheduling2022} integrated machine-learned ETA into robust berth scheduling, Lei et al.~\cite{leiPredictingVesselArrival2024} incorporated inland-waterway factors through tree-based stacking, Chu et al.~\cite{chuVesselArrivalTime2025} fused port-call records with AIS trajectories, and Saber et al.~\cite{saberHighaccuracyPredictionVessels2025} combined Extra Trees and LightGBM for seaport ETA prediction.
Deep learning has also been explored in this branch: El Mekkaoui et al.~\cite{elmekkaouiDeepLearningModels2022} compared CNN, LSTM, ANN, and RNN models for bulk-port ETA prediction, and Du et al.~\cite{duSegmentedETAPrediction2025} proposed a segmented deep-learning approach using AIS and pilotage records.
These studies confirm that trajectory states, operational covariates, and feature engineering are central to vessel ETA prediction.

\subsection{Research Gaps in Existing ETA Prediction Studies}
The literature shows clear progress, but several gaps remain for full-voyage open-ocean ETA prediction.
First, path-based methods preserve route geometry, yet they often rely on relatively regular historical routes, port approaches, inland waterways, or route segments close to the destination.
Second, trajectory-tracking methods achieve practical performance across many operational settings, but spatial information is usually represented as input features or temporal states rather than as an explicit geospatial structure.
Third, existing studies are fragmented across port, inland-waterway, pilotage, and near-arrival scenarios, making it difficult to compare representative prediction pipelines under a shared full-voyage protocol.
These gaps motivate our study of trans-Pacific vessel ETA prediction as geospatial forecasting over AIS trajectories, where ocean grid cells, observed AIS transitions, and geographic adjacency are used to construct a route graph and evaluate which modeling ingredients are useful for long-distance maritime ETA prediction.
...
\begin{table*}[t]
    \centering
    \caption{Scope of recent maritime ETA prediction studies. The table positions this work by route setting and prediction scope; numerical results are not compared because datasets, target definitions, prediction horizons, and evaluation protocols differ.}
    \label{tab:lit_comparison}
    \footnotesize
    \setlength{\tabcolsep}{3pt}
    \renewcommand{\arraystretch}{1.08}
    \begin{tabular}{P{2.5cm} P{2.0cm} P{2.0cm} P{2.7cm} P{5.0cm}}
        \toprule
        \textbf{Study} & \textbf{Route setting} & \textbf{Prediction scope} & \textbf{Modeling focus} & \textbf{Relation to this work} \\
        \midrule
        Yu et al.~\cite{yuShipArrivalPrediction2018} & Port call & Short range & RF / ANN / CART & Operational arrival prediction near port. \\
        Kolley et al.~\cite{kolleyRobustBerthScheduling2022} & European port & Short range & ML ETA for scheduling & Integrates ETA uncertainty into berth-planning decisions. \\
        El~Mekkaoui et al.~\cite{elmekkaouiDeepLearningModels2022} & Bulk port & Short--medium range & CNN / LSTM / ANN / RNN & Deep-learning benchmark in a port-centered setting. \\
        Du et al.~\cite{duSegmentedETAPrediction2025} & Pilotage area & Short range & Segmented deep model & Focuses on near-port/pilotage-stage ETA. \\
        Lei et al.~\cite{leiPredictingVesselArrival2024} & Inland waterway & Short range & Tree-based stacking & Uses structured operational and waterway factors. \\
        Saber et al.~\cite{saberHighaccuracyPredictionVessels2025} & Seaport & Medium range & Ensemble trees & Emphasizes high-accuracy port ETA with tabular fusion. \\
        Chu et al.~\cite{chuVesselArrivalTime2025} & Multi-port & Medium range & Stacked ensemble & Combines port-call records with AIS trajectories. \\
        Lloret-Batlle et al.~\cite{lloretbatlle2025} & Cross-Pacific & Near-arrival segment & ANN & Studies cross-Pacific ETA, but at a shorter remaining-distance stage. \\
        \midrule
        \textbf{This work} & \textbf{Cross-Pacific} & \textbf{Full voyage} & \textbf{Route graph + benchmark + uncertainty} & \textbf{Evaluates full-voyage ETA pipelines over large-scale AIS trajectories and analyzes duration-dependent reliability.} \\
        \bottomrule
    \end{tabular}
\end{table*}
...
We studied full-voyage trans-Pacific vessel ETA prediction as a large-scale geospatial forecasting problem over sparse AIS trajectories, open-ocean corridors, and duration-dependent operational uncertainty. Using 35.9 million AIS position reports and 9,087 voyages, we evaluated ten representative ETA prediction pipelines across five model families under a shared preprocessing, splitting, and evaluation protocol. This setting allows us to examine not only average prediction accuracy, but also how route-aware spatial structure, statistical feature design, ensemble knowledge transfer, and uncertainty quantification contribute to long-distance maritime ETA prediction.

First, the results show that strong statistical representations remain difficult to outperform in open-ocean ETA prediction. XGBoost achieves competitive single-model performance with an MAE of 15.14\,h, while the proposed MSTGN with enriched statistical features, GCN-derived spatial context, and ensemble knowledge distillation achieves the best single-model result with an MAE of 15.13\,h. The teacher ensemble, MSTGN-Ens7, obtains the lowest overall error with an MAE of 14.95\,h, suggesting that multi-seed ensemble diversity provides useful supervision for compressing ensemble behavior into a deployable single model.

Second, the ablation studies clarify the role of spatial modeling in this task. MSTGN is best interpreted as a graph-augmented feature-fusion framework rather than a purely end-to-end spatio-temporal sequence model. Its route graph represents ocean space through grid cells connected by observed AIS transitions and geographic adjacency, and the GCN branch supplies spatial context embeddings that complement hand-crafted voyage statistics. At the same time, statistical feature engineering remains the dominant predictive component: a simple MLP using 44-dimensional statistical features achieves 15.41\,h MAE and outperforms all GRU-based sequence models (${\geq}16.03$\,h). Meteorological features further improve MAE by 7.1\%, whereas asymmetric loss functions degrade accuracy by up to 17.6\%, and self-distillation from a single model is ineffective in this setting.

Third, our reliability analysis shows that operational ETA prediction should include calibrated uncertainty estimates rather than relying only on point predictions. Ensemble-based conformal prediction achieves 89.9\% empirical coverage at the 90\% target level while producing intervals that are 8\% narrower than fixed-width alternatives. The resulting duration-specific buffer times, ranging from 1.5\,h for predictions shorter than one day to 67.8\,h for predictions longer than 14 days, provide practical guidance for port scheduling and risk-aware maritime operations.

Overall, these findings suggest that full-voyage maritime ETA prediction benefits from treating vessel movement as geospatial forecasting over route-structured AIS trajectories. For practical deployment, robust statistical features, route-aware spatial context, ensemble knowledge distillation, and conformal uncertainty quantification should be considered jointly. Future work will incorporate ocean current data, investigate multi-step prediction horizons where temporal dynamics may become more important, extend the evaluation to global shipping routes, and improve long-tail calibration through duration-conditioned conformal procedures.
