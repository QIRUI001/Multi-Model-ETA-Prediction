...
Our results reveal that XGBoost achieves the best overall performance (MAE = 15.14\,h), while our MSTGN-MLP variant achieves the best neural network performance (MAE = 15.40\,h, 2.1\% lower than the MLP baseline).
...
These findings reveal that for single-step vessel ETA prediction, compact statistical features outperform both raw sequential input and complex spatio-temporal architectures.
\end{abstract}
...
\begin{enumerate}
    ...
    \item A novel Maritime Spatio-Temporal Graph Network (MSTGN) that constructs a route graph over ocean grid cells and fuses GCN-derived spatial context with statistical features, achieving the best neural network performance (MAE = 15.40\,h).
...
\begin{table}[t]
    ...
    \begin{tabular}{llccc}
        ...
        \midrule
        \multirow{3}{*}{Graph} & MSTGN-MLP & \underline{15.40} & \underline{26.57} & 9.50 \\
...
XGBoost achieves the lowest MAE (15.14\,h), RMSE (26.05\,h), and MAPE (9.33\%), outperforming all neural models despite operating on hand-crafted statistical features rather than raw sequential input.
This result aligns with recent findings in time-series forecasting~\cite{zeng2023transformers} that simpler models can rival or surpass complex architectures when strong engineered features are available.

\paragraph{Finding 2: MSTGN-MLP is the best neural model.}
Our proposed MSTGN-MLP achieves MAE = 15.40\,h, the best among all neural models and only 1.7\% behind XGBoost.
Notably, this model does \textit{not} use sequence modeling---it operates on the same 44-dimensional statistical features as XGBoost, augmented with 32-dimensional GCN-derived cell embeddings.
The MSTGN-LateFusion variant (GRU + graph, 15.56\,h) also outperforms all non-graph neural baselines, confirming the spatial embeddings provide useful complementary signal.
...
First, XGBoost achieves the best overall performance (MAE = 15.14\,h), while our proposed MSTGN-MLP achieves the best neural network performance (MAE = 15.40\,h, only 1.7\% behind XGBoost) by combining statistical features with GCN-derived spatial embeddings from a maritime route graph.
...
This suggests that for single-step ETA prediction with strong positional features, the bottleneck lies in feature informativeness rather than model complexity.
...
These findings provide practical guidance for maritime stakeholders: invest in feature engineering and tabular models before pursuing complex deep architectures.
