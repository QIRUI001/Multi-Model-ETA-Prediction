...
Our results reveal that XGBoost achieves the best performance (MAE = 15.14\,h), followed closely by neural sequence models, while the Informer---despite its architectural sophistication---does not outperform simpler alternatives.
...
\begin{table}[t]
    ...
    \begin{tabular}{llccc}
        ...
        \midrule
        \multirow{2}{*}{Convolutional} & 1D-CNN & -- & -- & -- \\
        & TCN & -- & -- & -- \\
        \midrule
        Tree & XGBoost & \textbf{15.14} & \textbf{26.05} & \textbf{9.33} \\
        \midrule
        \multirow{3}{*}{Attention} & Transformer & -- & -- & -- \\
        & Conv-Trans. & -- & -- & -- \\
...
\begin{figure}[t]
    ...
    \caption{MAE comparison across all ten methods, grouped by architectural family. Ridge Regression (58.30\,h) is omitted for visual clarity. The dashed line indicates the best result (XGBoost, 15.14\,h).}
...
Excluding Ridge, all models achieve MAEs within a narrow 15--17\,h range (${<}15\%$ relative spread), despite spanning five distinct architectural families and parameter counts from 270K to 11.3M.
...
\paragraph{Finding 3: The Informer does not benefit from its complexity.}
The Informer (11.3M parameters) achieves a higher MAE than simpler models with 7--40$\times$ fewer parameters.
Its ProbSparse attention and encoder distilling mechanisms, designed for long-horizon multi-step forecasting, may be unnecessary for the single-step prediction task considered here ($L_{\text{pred}} = 1$).

\paragraph{Finding 4: Neural sequence models perform comparably.}
LSTM, GRU, MLP, and the attention-based models all cluster within a narrow performance band.
No single neural architecture family demonstrates a clear advantage over others, suggesting that the bottleneck lies in feature informativeness rather than temporal modeling capability.
...
Finally, the narrow performance spread among neural models (15.73--16.20\,h MAE) suggests that the bottleneck in vessel ETA prediction lies in feature informativeness and data quality rather than model architecture.
